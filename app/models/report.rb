...
=end
  ...
  def self.tb_treatment_outcome(start_date, end_date, age)
    tb_program_id = Program.find_by_name('TB PROGRAM').id
    tb_drugs = []
    tb_regimens = Regimen.find(	:all,
									:order => 'regimen_index',
									:conditions => ['program_id = ?', tb_program_id])

    tb_regimens.each do |tb_regimen|
      (tb_regimen.regimen_drug_orders || []).each do |drug_order|
        tb_drugs << drug_order.drug_inventory_id
      end
    end
   ...
   total_registered = {}
    ...
    result = Encounter.find_by_sql("SELECT * FROM earliest_start_date e
      ...
      INNER JOIN orders o ON obs.order_id = o.order_id INNER JOIN drug_order do ON
      o.order_id = do.order_id INNER JOIN drug d ON do.drug_inventory_id = d.drug_id
      AND do.drug_inventory_id IN (#{tb_drugs.uniq.join(', ')}) AND obs.concept_id = #{amount_dispensed_concept}
      AND p.voided = 0 WHERE e.earliest_start_date BETWEEN '#{start_date}' AND '#{end_date}'
      AND (LEFT(e.date_enrolled,10) = e.earliest_start_date)
      AND age_at_initiation BETWEEN #{age.first} AND #{age.last}
      GROUP BY e.patient_id")
