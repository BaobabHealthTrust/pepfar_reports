class ReportsController < ApplicationController
  ...
  def on_art_started_ipt
    if not params[:start_date].blank? and not params[:end_date].blank?
      ...
      @total_registered = Report.on_art_started_ipt(@start_date, @end_date, age)
      @followup_months = {}
      unless @total_registered.blank?
        patients_to_follow = @total_registered.keys
        (1.upto(12)).each do |m|
  ...
  def on_art_with_tb
    if not params[:start_date].blank? and not params[:end_date].blank?
      ...
      @total_registered = Report.on_art_with_tb_symptoms(@start_date, @end_date, age)
      @followup_months = {}
      unless @total_registered.blank?
        patients_to_follow = @total_registered.keys
        (1.upto(12)).each do |m|
